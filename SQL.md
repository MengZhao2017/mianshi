...
--------------------------------------------------------------------------------------------------------------------------------------
   ...
   什么是索引？https://blog.csdn.net/weiliangliang111/article/details/51333169
...
聚集索引  https://blog.csdn.net/jiadajing267/article/details/54581262
...
--------------------------------------------------------------------------------------------------------------------------------------
 ...
 1.每个节点最多有m个分支（m是树的阶数：所有节点个数的最大值）,每个节点的关键字的个数n: m/2(上取整)-1<=n<=m-1,根节点关键字个数为：1<=n<=m-1
 
 2.n个关键字有n+1个分支
 
 3.左节点内的所有关键字均小于右边节点内的关键字
 
 4.节点内各个关键字互不相等，且从小到大排列
 ...
 2.B+树叶子节点包含信息，有记录，叶子节点关键字可以作为记录的指针
 
 3.非叶子节点仅仅起到一个索引的作用
 
 4.B+树上有一个指针指向关键字最小的叶子节点，所有叶子节点链接成一个线性链表
 
 5.每个节点的关键字的个数n: m/2(上取整)<=n<=m,根节点关键字个数为：1<=n<=m
